  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<TransactionWithItems>>(
      ...
      builder: (context, snapshot) {
        ...
        final income = items.where((item) => item.transaction.amount > 0).length;
        final expense = items.where((item) => item.transaction.amount < 0).length;
              ...
              actions: [
                IconButton(
                  ...
                  onPressed: () => showAppInfoDialog(
                    ...
                    message:
                        'Gunakan kategori, toko, tag, dan tanggal untuk melihat kebiasaan keuangan keluarga. Semua angka di sini berasal dari transaksi yang tersimpan di perangkat.',
                  ),
        ...
        if (items.isEmpty)
          const Padding(
            ...
            child: AppEmptyState(
              ...
              message:
                  'Catat beberapa transaksi dulu. Nanti analisa akan mulai menunjukkan perbandingan nyata.',
            ),
          ),
      ],
    );
