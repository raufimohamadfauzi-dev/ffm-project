...
class _FlexibleCashFlowPageState extends State<FlexibleCashFlowPage> {
  ...
  Future<void> _showCreateOrEditDialog([CashFlowProfile? existing]) async {
    ...
    final capitalCtrl = TextEditingController(
      text: existing != null
          ? _formatNumber(existing.initialCapital)
          : '15.000.000',
    );
    final inflowCtrl = TextEditingController(
      text: existing != null
          ? _formatNumber(existing.estimatedInflow)
          : '45.000.000',
    );
    final livingCtrl = TextEditingController(
      text: existing != null
          ? _formatNumber(existing.dailyLivingBudget)
          : '85.000',
    );
    final operationalCtrl = TextEditingController(
      text: existing != null
          ? _formatNumber(existing.dailyOperationalBudget)
          : '50.000',
    );
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isEditing
                          ? 'Ubah Siklus Arus Kas'
                          : 'Buat Siklus Baru (AgroTrack / Bisnis)',
                        ...
                        Expanded(
                          child: InkWell(
                            ...
                            onTap: () async {
                              final picked = await showDatePicker(
                                context: context,
                                initialDate: harvestDate.isBefore(startDate)
                                    ? startDate
                                    : harvestDate,
                                firstDate: startDate,
                                lastDate: DateTime(2035),
                      ...
                      child: FilledButton(
                        onPressed: () async {
                          ...
                          final newProfile = CashFlowProfile(
                            id:
                                existing?.id ??
                                'cycle_${DateTime.now().millisecondsSinceEpoch}',
                            ...
                            dailyOperationalBudget: _parseCurrency(
                              operationalCtrl.text,
                            ),
                        ...
                        child: Text(
                          isEditing ? 'Simpan Perubahan' : 'Mulai Siklus Ini',
                        ),
                      ),
                    ),
                  ],
      ...
      builder: (ctx) {
        return StatefulBuilder(
          builder: (ctx, setDialogState) {
            ...
            final alloc = _calculator.simulateHarvestAllocation(
              actualHarvestRevenue: revenue,
            );
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  _buildAllocRow(
                    'Modal Siklus Berikutnya (45%)',
                    alloc['nextCycleCapital'] ?? 0,
                    Colors.blue,
                  ),
                  const SizedBox(height: 6),
                  _buildAllocRow(
                    'Cadangan Dana Darurat (15%)',
                    alloc['emergencyFund'] ?? 0,
                    Colors.orange,
                  ),
                  const SizedBox(height: 6),
                  _buildAllocRow(
                    'Laba Bersih Keluarga (40%)',
                    alloc['familyProfit'] ?? 0,
                    Colors.green,
                  ),
                ],
              ),
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Text(
                              'Rp ${_formatNumber(p.initialCapital)}',
                              style: const TextStyle(
                                fontWeight: FontWeight.bold,
                              ),
                            ),
                        ...
                        Row(
                          ...
                          children: [
                            Text(
                              'Estimasi Inflow / Hasil:',
                              style: theme.textTheme.bodySmall,
                            ),
                            Text(
                              'Rp ${_formatNumber(p.estimatedInflow)}',
                              style: TextStyle(
                                fontWeight: FontWeight.bold,
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Text(
                              '${p.daysElapsed} dari ${p.totalDays} hari (Sisa ${p.daysRemaining} hari)',
                              style: const TextStyle(
                                fontWeight: FontWeight.bold,
                              ),
                            ),
