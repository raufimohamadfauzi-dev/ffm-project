...
class FinancialHealthCalculator {
  ...
  FinancialHealthScore calculate(FinancialHealthInput input) {
    ...
    if (input.totalMonthlyInstallments > 0 && input.totalIncome > 0) {
      final installmentRatio =
          input.totalMonthlyInstallments / input.totalIncome;
      if (installmentRatio <= .3) score += 10;
      if (installmentRatio > .5) score -= 15;
    }
    if (input.averageMonthlyExpenses > 0 &&
        input.emergencyFundAmount >= input.averageMonthlyExpenses * 3) {
      score += 10;
    } else if (input.averageMonthlyExpenses > 0 &&
        input.emergencyFundAmount < input.averageMonthlyExpenses) {
      score -= 5;
    }
