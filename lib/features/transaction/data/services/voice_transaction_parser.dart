...
abstract final class VoiceTransactionParser {
  static List<VoiceTransactionParseResult> parseMany(
    ...
    final chunks = transcript
        .split(
          RegExp(
            r'\s+(?:lalu|kemudian|dan juga|setelah itu)\s+|[;\n]+',
            caseSensitive: false,
          ),
        )
        .map((item) => item.trim())
  ...
  static VoiceTransactionParseResult parse(
    ...
    final income = RegExp(
      r'\b(pemasukan|masuk|menerima|diterima|gaji|pendapatan|dapat)\b',
    ).hasMatch(lower);
    final expense = RegExp(
      r'\b(pengeluaran|keluar|bayar|beli|belanja|jajan|biaya)\b',
    ).hasMatch(lower);
