...
class FfmAssistantFeedbackRepository {
  ...
  Future<List<FfmAssistantFeedback>> getFeedbackByType(
    ...
    final allFeedback = await getAllFeedback();
    return allFeedback.where((feedback) => feedback.type == type).toList();
  }
  ...
  Future<void> cleanupOldFeedback({
    ...
    feedbackList.removeWhere(
      (feedback) => feedback.createdAt.isBefore(cutoffDate),
    );
  ...
  Future<Map<String, int>> getFeedbackStats() async {
    ...
    for (final feedback in allFeedback) {
      ...
      stats['hallucination'] =
          ...
          (feedback.category == FfmAssistantFeedbackCategory.hallucination
              ? 1
              : 0);
      stats['missingContext'] =
          (stats['missingContext'] ?? 0) +
          (feedback.category == FfmAssistantFeedbackCategory.missingContext
              ? 1
              : 0);
      stats['other'] =
          (stats['other'] ?? 0) +
