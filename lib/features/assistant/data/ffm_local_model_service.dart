...
class FfmLocalModelService {
  ...
  Future<void> importSingleGguf(
    PlatformFile selected, {
    void Function(FfmModelProgress progress, String stage)? onProgress,
  }) async {
    ...
    final totalBytes = selected.path != null && File(selected.path!).existsSync()
        ? File(selected.path!).lengthSync()
        : 0;
    IOSink? sink;
    try {
      onProgress?.call(
        ...
        await for (final chunk in selected.readAsByteStream()) {
          ...
          onProgress?.call(
            FfmModelProgress(
              fileName: fileName,
              receivedBytes: processed,
              totalBytes: totalBytes > 0 ? totalBytes : processed,
            ),
            'Menyalin berkas GGUF...',
          );
        }
      ...
      onProgress?.call(
        FfmModelProgress(
          fileName: fileName,
          receivedBytes: 0,
          totalBytes: await temporary.length(),
        ),
        'Memverifikasi hash SHA-256...',
      );

      final hash = await checksum(
        temporary,
        onProgress: (processed, total) {
          onProgress?.call(
            FfmModelProgress(
  ...
  Future<FfmLocalModelInfo> importBundle(
    File bundleFile, {
    void Function(FfmModelProgress progress, String stage)? onProgress,
  }) async {
  ...
  Future<FfmLocalModelInfo?> pickAndInstallBundle({
    void Function(FfmModelProgress progress, String stage)? onProgress,
  }) async {
    ...
    return _importPickedFile(selected, onProgress: onProgress);
  }

  Future<FfmLocalModelInfo> _importPickedFile(
    PlatformFile selected, {
    void Function(FfmModelProgress progress, String stage)? onProgress,
  }) async {
    final sourcePath = selected.path;
    if (sourcePath != null) {
      return importBundle(File(sourcePath), onProgress: onProgress);
    }
    ...
    try {
      ...
      await for (final chunk in selected.readAsByteStream()) {
        ...
        onProgress?.call(
          ...
          'Menyalin bundle offline...',
        );
      }
      await sink.flush();
      await sink.close();
      sink = null;
      return await importBundle(temporary, onProgress: onProgress);
    } finally {
      await sink?.close();
      await _deleteIfExists(temporary);
    }
  ...
  Future<void> _extractStoreEntry(
    ...
    try {
      ...
      final total = entry.uncompressedSize;
      while (remaining > 0) {
        ...
        onProgress?.call(total - remaining, total);
      }
      await sink.flush();
      await sink.close();
      sink = null;
