...
class GeminiImageInput {
  const GeminiImageInput({required this.base64Data, required this.mimeType});
...
class GeminiService {
  ...
  Future<List<GeminiModelOption>> listModels({required String apiKey}) async =>
      (await fetchModels(apiKey: apiKey)).models;
  ...
  Future<GeminiResult> _generate({
    ...
    try {
      final imageList =
          images ?? (image == null ? const <GeminiImageInput>[] : [image]);
      final response = await _client
          .post(
            url,
            headers: {
      ...
      try {
        ...
        if (decoded is Map<String, dynamic> &&
            decoded['usageMetadata'] is Map<String, dynamic>) {
          usageMetadata = GeminiUsageMetadata.fromJson(
      ...
      if (text == null && functionCalls == null) {
        return GeminiResult(
          model: model,
          statusCode: response.statusCode,
      ...
      var finalText = text;
      if (finalText != null &&
          _proposalGateDenied(systemInstruction) &&
          _containsStructuredProposal(finalText)) {
      ...
      return GeminiResult(
        ...
        statusCode: null,
        message: isTimeout
            ? 'Request Gemini timeout setelah 60 detik. Periksa stabilitas koneksi internet.'
  ...
  bool _proposalGateDenied(String? instruction) =>
      instruction?.contains('MUTATION_PROPOSAL_GATE: DENY') ?? false;
  bool _containsStructuredProposal(String text) =>
      text.contains('ffm-assistant-proposal-v1') ||
      RegExp(r'"(?:proposal|clarification)"\s*:').hasMatch(text);
  ...
  String _statusMessage(int statusCode, [String? body]) {
    ...
    if (lowerBody.contains('api key not valid') ||
      ...
      return 'Kunci API Gemini tidak valid. Silakan periksa kembali di Pengaturan.';
    }
    return switch (statusCode) {
      ...
      404 =>
        'Model Gemini tidak ditemukan. Coba pilih model lain di Pengaturan.',
