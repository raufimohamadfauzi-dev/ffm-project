...
class JsonBackupService {
  ...
  BackupPreview previewJson(String content) {
    ...
    if (modules is! Map) {
      return BackupPreview(formatVersion: version, isFull: false, counts: const {});
    }
  ...
  Future<void> importAndRestore(String path) async {
    ...
    for (final table in _tables) {
      ...
      modules[table] = rows.whereType<Map>().map((row) => Map<String, dynamic>.from(row)).toList();
    }
