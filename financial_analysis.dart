...
class WeeklyDigest {
  const WeeklyDigest({required this.expense, required this.income, required this.transactionCount});
  final int expense;
  final int income;
...
abstract final class FinancialAnalysis {
  static List<ForecastPoint> forecast(
    ...
    final average = (values.fold<int>(0, (sum, value) => sum + value) / values.length).round();
  ...
  static WeeklyDigest weeklyDigest(
    ...
    final start = DateTime(weekContaining.year, weekContaining.month, weekContaining.day)
    ...
    return WeeklyDigest(expense: expense, income: income, transactionCount: count);
  }
}
