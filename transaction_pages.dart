...
class _TransactionListPageState extends State<TransactionListPage> {
  ...
  Future<void> _loadTransactions() async {
    ...
    final merchants = await (getIt<AppDatabase>().select(
      getIt<AppDatabase>().merchants,
    )..orderBy([(table) => OrderingTerm.asc(table.name)])).get();
  ...
  Future<void> _openTransfer() async {
    ...
    if (draft.adminFee > 0) {
      feeCategory = await (database.select(
        database.categories,
      )..where((row) => row.id.equals('cat-biaya-admin'))).getSingleOrNull();
      if (feeCategory == null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
    ...
    await database.transaction(() async {
      ...
      for (final rawTag in draft.tags) {
        ...
        final existing = await (database.select(
          database.tags,
        )..where((table) => table.name.equals(tagName))).getSingleOrNull();
        final tagId = existing?.id ?? const Uuid().v4();
        if (existing == null) {
          await database
              .into(database.tags)
              .insert(TagsCompanion.insert(id: tagId, householdId: AppContext.householdId, name: tagName, createdAt: DateTime.now()));
        }
        await database
  ...
  int _goalContribution(TransactionEntity? transaction) {
    if (transaction == null ||
  ...
  Future<void> _applyGoalDelta(String? goalId, int delta) async {
    if (goalId == null || delta == 0) return;
    final goal = await getIt<GetGoal>()(AppContext.householdId, goalId);
    if (goal == null) return;
  ...
  Future<void> _syncGoalContribution({
    ...
    final oldContribution = _goalContribution(previous);
    final newContribution = nextGoalId == null || nextAmount >= 0
        ? 0
        : nextAmount.abs();
    if (oldGoalId == nextGoalId) {
      await _applyGoalDelta(oldGoalId, newContribution - oldContribution);
      return;
    }
    await _applyGoalDelta(oldGoalId, -oldContribution);
    await _applyGoalDelta(nextGoalId, newContribution);
  }
      ...
      body: _loading
          ...
          ? ListView(
              ...
              children: [
                const AppHelpBanner(
                  title: 'Cara pakainya',
                  message: 'Sebaiknya mulai dari pemasukan awal supaya saldo keluarga punya titik awal. Setelah itu catat pengeluaran dan lihat dampaknya. Kalau punya nota, pakai Scan Nota lalu cek hasilnya sebelum menyimpan.',
                ...
                AppEmptyState(
                  ...
                  message: 'Sebaiknya catat pemasukan awal dulu. Setelah saldo ada, pengeluaran akan terlihat jelas mengurangi saldo keluarga.',
                  action: FilledButton.icon(
                    onPressed: _openForm,
                    icon: const Icon(Icons.add),
                    label: const Text(AppCopy.tambahTransaksi),
                  ),
                ),
              ],
            )
...
class _TransactionFormPageState extends State<TransactionFormPage> {
  ...
  void initState() {
    ...
    final total =
        scan.total ?? scan.items.fold<int>(0, (sum, item) => sum + item.price);
  ...
  Future<void> _loadMasterTags() async {
    final tags = await (getIt<AppDatabase>().select(
      getIt<AppDatabase>().tags,
    )..orderBy([(table) => OrderingTerm.asc(table.name)])).get();
    if (!mounted) return;
  ...
  Future<void> _showTagInfo() async {
    await showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Tag adalah penanda tambahan untuk mengelompokkan transaksi lintas kategori. Contohnya wajib, sekolah, kerja, mudik, atau bisa ditunda. Tag tidak mengubah saldo dan tidak menggantikan kategori. Kamu bisa memilih tag dari Data Utama atau mengetik tag baru.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _chooseGoal(List<GoalEntity> activeGoals) async {
      ...
      builder: (sheetContext) {
        ...
        return SafeArea(
          child: ListView(
            ...
            children: [
              Text(
                'Pilih target setoran',
                style: Theme.of(sheetContext).textTheme.titleLarge,
              ),
              const SizedBox(height: 4),
              Text(
                'Transaksi tetap mengurangi saldo keluarga. Pilihan ini cuma menambah progres target.',
              ...
              ListTile(
                ...
                title: const Text('Tidak disetor ke target'),
                subtitle: const Text('Simpan sebagai transaksi biasa saja'),
                onTap: () => Navigator.pop(sheetContext, _noGoalSelection),
              ),
              ...activeGoals.map(
                (goal) => ListTile(
                  minVerticalPadding: 10,
                  ...
                  subtitle: Text(
                    '${formatRupiahInput(goal.currentAmount.toString())}/${formatRupiahInput(goal.targetAmount.toString())} terkumpul',
                  ),
                  trailing: goal.id == _goalId
                      ? Icon(Icons.check_circle, color: scheme.primary)
                      : null,
                  onTap: () => Navigator.pop(sheetContext, goal.id),
                ),
              ),
            ],
          ),
    ...
    setState(() {
      _goalId = selected == _noGoalSelection ? null : selected;
    });
  }
  ...
  Widget _buildAccountSelector() {
    ...
    return SearchableDropdown(
      items: _accounts,
      selectedItem: selectedAccounts.isEmpty ? null : selectedAccounts.first,
      itemLabel: (account) =>
          '${account.name} · ${_accountTypeLabel(account.type)}',
      itemId: (account) => account.id,
      labelText: 'Rekening atau dompet',
      helperText:
          'Tempat uang masuk atau keluar. Bukan pemilik saldo dan bukan kategori hutang.',
  ...
  Widget _buildGoalSelector(BuildContext context) {
      ...
      return AppCard(
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                'Belum ada target yang bisa disetor. Bikin target dulu kalau mau memantau uang terkumpulnya.',
                style: Theme.of(context).textTheme.bodySmall,
              ),
            ),
            TextButton(
              onPressed: _openGoalForm,
              child: const Text('Bikin target'),
            ),
          ],
        ),
      );
    ...
    return AppCard(
      ...
      child: ListTile(
        ...
        title: const Text('Setor ke Target (opsional)'),
        subtitle: Text(
          selectedGoal == null
              ? 'Belum dipilih. Ketuk untuk memilih target.'
              : '${selectedGoal.name} · ${formatRupiahInput(selectedGoal.currentAmount.toString())}/${formatRupiahInput(selectedGoal.targetAmount.toString())} terkumpul. Kategori tetap kamu pilih sendiri.',
          maxLines: 2,
          overflow: TextOverflow.ellipsis,
        ),
        trailing: const Icon(Icons.chevron_right),
        onTap: () => _chooseGoal(activeGoals),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final flowColor = isIncome ? scheme.tertiary : scheme.error;
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            AppCard(
              color: flowColor.withValues(alpha: .09),
                  ...
                  children: [
                    ...
                    if (!_listening && voiceResult != null) ...[
                      ...
                      _VoiceResultLine(
                        ...
                        value: voiceResult.partyName ?? voiceResult.owner ?? 'Belum diatur',
            ...
            const AppSectionHeader(
              ...
              helpText: 'Isi kategori dan sumber saldo supaya transaksi mudah dibaca dan dicocokkan.',
            ),
            const SizedBox(height: 8),
            ...
            TextButton.icon(
              ...
              label: const Text('Atur kategori, toko, tag, dan sumber saldo'),
            ),
            const SizedBox(height: 16),
            ...
            if (_merchants.isNotEmpty) ...[
              ...
              SearchableDropdown(
                ...
                helperText:
                    'Pilih dari Data Utama agar nama dan rinciannya konsisten.',
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Misalnya Pasar Pagi, rumah, atau kantor',
                helperText: 'Rekening = tempat uang berada. Lokasi = tempat kejadian transaksi.',
            ...
            const AppSectionHeader(
              ...
              helpText: 'Tambahkan tanggal, penanda pemakaian, catatan, tag, atau lampiran bila memang diperlukan.',
            ),
            const SizedBox(height: 4),
            ListTile(
              contentPadding: EdgeInsets.zero,
            ...
            if (isDataSusulan(_date, now: DateTime.now()))
              Padding(
                ...
                child: Text(
                  'Tanggal kejadian berbeda dari hari ini. Setelah disimpan, data ini akan ditandai sebagai data susulan.',
                  style: Theme.of(context).textTheme.bodySmall
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Tag transaksi',
                      hintText: 'Contoh: wajib, sekolah, akhir bulan',
                      helperText: 'Tag membantu menyaring dan membaca pola transaksi. Ketik sendiri atau pilih dari Data Utama.',
                    ...
                    Wrap(
                      ...
                      children: [
                        for (final tag in _masterTags)
                          Tooltip(
                            message: 'Tag ini berasal dari Data Utama.',
                            child: ActionChip(
                              label: Text('#${tag.name}'),
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Tag',
                helperText: 'Pisahkan beberapa tag dengan koma.',
              ),
            ),
          ],
        ),
...
class _VoiceReviewDialogState extends State<_VoiceReviewDialog> {
  ...
  void _addTag(String rawTag) {
    ...
    if (tag.isEmpty ||
        _tags.any((current) => current.toLowerCase() == tag.toLowerCase())) {
      _tagController.clear();
      return;
    }
    setState(() {
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Misalnya sekolah atau wajib',
              ),
            ),
            if (widget.tagsMaster.isNotEmpty) ...[
              const SizedBox(height: 8),
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              labelText: row.type == TransactionType.income ? 'Sumber pemasukan (opsional)' : 'Dipakai oleh (opsional)',
