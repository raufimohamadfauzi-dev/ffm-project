...
| Memori dan konteks Asisten | Simpan satu panggilan/preferensi melalui persetujuan eksplisit, tutup-buka aplikasi, lalu ajukan pertanyaan lanjutan yang relevan. Reset chat dan ulangi pertanyaan. | Memori yang disetujui dapat dipakai secara lokal; follow-up hanya memakai konteks percakapan ringkas; reset chat menghapus konteks sementara tetapi tidak menghapus memori yang disetujui; tidak ada detail PIN, respons mentah, atau draft finansial yang muncul sebagai memori. |
| Halaman sensitif dan fallback konteks | Buka Pengaturan PIN atau Privasi, lalu tanyakan hal umum; ulangi saat model belum siap atau setelah menutup-buka aplikasi. | Asisten tetap memberi fallback lokal yang jujur; detail halaman sensitif tidak disisipkan ke konteks model; aplikasi tidak menunggu tanpa batas dan tidak memakai layanan cloud. |
