...
void main() {
  ...
  group('VehicleRepository CRUD & Matching Tests', () {
    test('Menyimpan, membaca, dan menghapus kendaraan', () async {
      ...
      // Matching by plate
      final matchedByPlate = await repo.findVehicleByPlate(
        householdId,
        'b-1234-abc',
      );
      ...
      // Matching by keyword
      final matchedByKeyword = await repo.findVehicleByNameOrKeyword(
        householdId,
        'vario',
      );
  ...
  group(
    'Safe Restore (Merge & Append) Tests - Jangan Dihapus tapi Ditambahkan',
    () {
      test('importRaw menggabungkan kendaraan baru dan riwayat BBM tanpa menghapus data HP', () async {
        ...
        final beat = afterRestore.firstWhere(
          (v) => v.plateNumber == 'B 9999 HP',
        );
        ...
        final truk = afterRestore.firstWhere(
          (v) => v.plateNumber == 'E 1122 GG',
        );
