...
void main() {
  ...
  group('FinancialAnalysisIntelligenceEngine - Burn Rate & Cashflow Projection', () {
    test('menghitung burn rate harian dan proyeksi akhir bulan secara deterministik', () {
      final now = DateTime(
        ...
        9,
        10,
      ); // Hari ke-10 bulan September (30 hari total)
      final txs = [
        // Pemasukan 10 juta
        createTx(
          amount: 10000000,
    ...
    test(
      'mendeteksi potensi defisit jika pengeluaran harian melampaui pemasukan',
      () {
  ...
  group(
    'FinancialAnalysisIntelligenceEngine - Micro-Expense Leak Detector',
    () {
      test(
        'mendeteksi transaksi mikro berulang (<50rb dan >=3x frekuensi)',
        () {
  ...
  group(
    'FinancialAnalysisIntelligenceEngine - Top Expense Categories (Pareto)',
    () {
      test(
        'menghitung distribusi persentase kategori pengeluaran terbesar',
        () {
  ...
  group('FinancialAnalysisIntelligenceEngine - AgroTrack Stress Test', () {
    test('menghitung ketahanan kas saat panen tertunda 14 hari', () {
      final profile = CashFlowProfile(
        ...
        targetHarvestDate: DateTime.now().add(
