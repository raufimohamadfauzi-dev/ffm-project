...
void main() {
  ...
  group('F4.1 - F4.5 Goal Evidence Evaluator (Deterministic & Grounded)', () {
    test(
      'Reports targetReached when current amount satisfies target',
      () async {
    ...
    test(
      'Evaluates onTrack when cashflow comfortably covers monthly allocation',
      () async {
        final now = DateTime(2026, 9, 12);
    ...
    test(
      'Evaluates behindSchedule when required monthly saving exceeds surplus',
      () async {
        final now = DateTime(2026, 9, 12);
  ...
  group('F4 Integration: Interpreter & Capability Registry', () {
    test(
      'Interpreter answers goal progress query with grounded evaluation',
      () async {
    ...
    test(
      'Capability adapter read.goal_evidence_evaluation returns evaluation',
      () async {
        final now = DateTime(2026, 9, 12, 10, 0);
        await database
            .into(database.goals)
