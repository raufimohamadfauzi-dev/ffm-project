...
void main() {
  group('Performance: riwayat 1.000 & 10.000 records', () {
    test('query halaman pertama, filter, dan scroll tetap cepat untuk 10.000 transaksi', () async {
      ...
      expect(
        insertWatch.elapsed.inSeconds,
        lessThan(30),
        reason:
            'insert 10.000 transaksi terlalu lambat: ${insertWatch.elapsed}',
      );
      ...
      expect(
        swFilter.elapsed.inMilliseconds,
        lessThan(2000),
      ...
      expect(count, 10000);
      expect(
        swScroll.elapsed.inMilliseconds,
        lessThan(15000),
    ...
    test(
      'querySessionsPage & queryDailyNotesPage stabil untuk 1.000 aktivitas',
      () async {
        final database = createInMemoryDatabaseForTests();
        addTearDown(database.close);
        ...
        expect(
          swKeyword.elapsed.inMilliseconds,
          lessThan(2000),
    ...
    test(
      'query halaman pertama & filter tetap cepat di 100.000 transaksi',
      () async {
        final database = createInMemoryDatabaseForTests();
        ...
        await database.transaction(() async {
          ...
          while (i < total) {
            ...
            for (var k = 0; k < count; k++) {
              ...
              final date = DateTime(2018, 1, 1).add(Duration(days: i % 2900));
              args
                ..add('bench-tx-$i')
                ..add(AppContext.householdId)
                ..add(i % 3 == 0 ? 'expense' : 'income')
                ..add(
                  i.isEven ? -(1000 * ((i % 50) + 1)) : (1000 * ((i % 50) + 1)),
                )
        ...
        expect(
          ...
          lessThan(60),
          reason:
              'insert 100.000 transaksi terlalu lambat: ${insertWatch.elapsed}',
        );
