...
void main() {
  ...
  setUp(() {
    ...
    getIt.registerSingleton<FfmAssistantResponseFeedbackRepository>(
      feedbackRepo,
    );
    ...
    getIt.registerSingleton<FfmAssistantUnansweredQuestionRepository>(
      unansweredRepo,
    );
  });
  ...
  group('FfmAssistantIssueLogPage Widget Tests', () {
    testWidgets(
      'Renders empty state when no issues or unanswered questions exist',
      (tester) async {
        await tester.pumpWidget(
          const MaterialApp(home: FfmAssistantIssueLogPage()),
        );
    ...
    testWidgets(
      'Renders reported issue and unanswered question with execution trace',
      (tester) async {
        // 1. Record a reported bad assistant response
        await feedbackRepo.record(
          questionText: 'Berapa sisa saldo kas bulan ini?',
          responseText:
              'Saldo kas Anda adalah Rp 50.000.000 (salah angka halusinasi).',
          ...
          issueMetadata: {
            ...
            'processTrace': {
              ...
              'elapsedMs': 245,
              'events': [
                {'label': 'Menyiapkan permintaan...', 'elapsedMs': 0},
                {
                  'label': 'Membaca ringkasan transaksi bulan ini',
                  'elapsedMs': 35,
                  'detail': 'read.summary capability',
                },
                {'label': 'Gemini Cloud selesai', 'elapsedMs': 240},
              ],
            },
          },
        );
        ...
        expect(
          find.text('Bagaimana cara bayar zakat mal untuk ternak kambing?'),
          findsOneWidget,
        );
        ...
        expect(
          find.text('Membaca ringkasan transaksi bulan ini'),
          findsOneWidget,
        );
    ...
    testWidgets('Displays Coba di Chat button and filters by status chips', (
      tester,
    ) async {
      await feedbackRepo.record(
      ...
      await unansweredRepo.record(rawQuestion: 'Bagaimana cara transfer uang?');
