...
void main() {
  group('MonthlyReportLearningMetrics Calculations', () {
    test('Kondisi Sehat: Rasio tabungan >= 20% dan cicilan aman <= 30%', () {
      ...
      expect(
        metrics.keyLearningPoints[0],
        contains('Rasio tabungan sehat di angka 30.0%'),
      );
      expect(
        metrics.keyLearningPoints[1],
        contains('Pos "Belanja Bulanan" mendominasi 50.0% pengeluaran'),
      );
      expect(
        metrics.keyLearningPoints[2],
        contains('Beban cicilan bulanan aman di angka 20.0%'),
      );
    });
    ...
    test('Kondisi Defisit & Cicilan Berlebih (>30%)', () {
      final metrics = MonthlyReportLearningMetrics.calculate(
        ...
        topExpenseCategories: [const MapEntry('Cicilan & Belanja', 6000000)],
      );
      ...
      expect(
        metrics.keyLearningPoints[0],
        contains('Arus kas mengalami defisit Rp 2000000'),
      );
      expect(
        metrics.keyLearningPoints[1],
        contains('Pos "Cicilan & Belanja" mendominasi 50.0% pengeluaran'),
      );
      expect(
        metrics.keyLearningPoints[2],
        contains('Perhatian: Beban cicilan (40.0%) di atas ambang aman 30%'),
      );
    });
    ...
    test('Kondisi Tanpa Pemasukan (Zero Income)', () {
      final metrics = MonthlyReportLearningMetrics.calculate(
        ...
        totalAssetsValue: 0,
        topExpenseCategories: [const MapEntry('Operasional', 2000000)],
      );
  ...
  group('FfmAssistantScreenContextPolicy with Report Page Summary', () {
    test(
      'forPrompt menyertakan dataSummary laporan lengkap untuk konteks LLM',
      () {
        final snapshot = FfmAssistantPageContextSnapshot(
          ...
          dataSummary: 'Laporan September 2026: Pemasukan Rp 10.000.000, Pengeluaran Rp 7.000.000, Surplus Rp 3.000.000. Rasio Tabungan: 30.0%, Rasio Cicilan: 20.0%. Pos Terbesar: Belanja Bulanan Rp 3.500.000. Evaluasi: Rasio tabungan sehat di angka 30.0%.',
        );
        ...
        expect(
          prompt,
          contains('Laporan September 2026: Pemasukan Rp 10.000.000'),
        );
