...
void main() {
  ...
  http.Response okResponse({Map<String, dynamic>? body, String? text}) =>
      http.Response(
        jsonEncode(body ?? {'ok': true, 'result': {'message_id': 1, 'text': text ?? ''}}),
        200,
      );

  group('TelegramDeliveryRepository', () {
    test('enqueue creates a pending row and deduplicates on deliveryId', () async {
    ...
    test('enqueue deduplicates on dedupeKey even with different deliveryId', () async {
      await repo.enqueue(
        deliveryId: 'd-2',
        householdId: 'house',
        operation: 'alert',
    ...
    test('pendingDue filters pending/failed retryable and due items, oldest first', () async {
      ...
      await repo.markFailed(enqueueAt(fixed.add(const Duration(minutes: 2))),
          error: 'permanent', retryable: false, at: fixed);
    ...
    test('claim is atomic: single winner, increments attempt, respects due time', () async {
      await repo.enqueue(
        deliveryId: 'c-1',
        householdId: 'house',
        operation: 'alert',
  ...
  group('TelegramDeliveryProcessor', () {
    test('sends a pending delivery and records status when HTTP 200', () async {
      ...
      expect(row!.status, 'sent');
      expect((await config.loadOperationalStatus()).lastDeliveryStatus,
    ...
    test('sent weekly.report completes the weekly claim and records sent time', () async {
      final config = await configRepo();
      final processor = await readyProcessor(
        config,
        client: MockClient((req) async => okResponse()),
  ...
  group('Durable transaction notification', () {
    test('saving a transaction enqueues a durable notification row', () async {
      ...
      final rows =
          await repo.recentDeliveries(householdId: 'house', limit: 10);
      expect(rows, hasLength(1));
    ...
    test('re-saving the same transaction stamps does not duplicate the message', () async {
      final config = await configRepo();
      final usecase = SaveTransaction(
        db,
        telegramConfigRepository: config,
      ...
      expect(
        await repo.pendingCount(householdId: 'house'),
        1,
      );
    });

    test('edit stamps produce a transaction.edit message while keeps the old one', () async {
      final config = await configRepo();
      final usecase = SaveTransaction(
        db,
        telegramConfigRepository: config,
      ...
      expect(
        rows.map((r) => r.operation).toSet(),
        {'transaction.new', 'transaction.edit'},
      );
      expect(
    ...
    test('notifications stay disabled when notifyOnNewTransaction is off', () async {
  ...
  group('Coordinator durable weekly report', () {
    test('uses the outbox and returns success when delivery is sent', () async {
      ...
      final coordinator = AutonomousEvaluationCoordinator(
        database: db,
        insightRepository: FfmAssistantInsightRepository(db, clock: () => fixed),
        clock: () => fixed,
      ...
      final rows =
          await repo.recentDeliveries(householdId: 'house-weekly', limit: 10);
      expect(rows, hasLength(1));
      expect(rows.single.operation, 'weekly.report');
      expect(rows.single.status, 'sent');
    });

    test('returns false and keeps the row failed when Telegram rejects', () async {
      final config = await configRepo();
      final processor = await readyProcessor(
        config,
        client: MockClient((req) async {
          return http.Response(
            jsonEncode({'ok': false, 'error_code': 400, 'description': 'Bad Request'}),
            400,
          );
        }),
      );
      ...
      final rows =
          await repo.recentDeliveries(householdId: 'house-fail', limit: 10);
      expect(rows, hasLength(1));
      expect(rows.single.operation, 'weekly.report');
  ...
  group('Credential fingerprint', () {
    test('recordVerificationResult stores fingerprint only on success', () async {
      SharedPreferences.setMockInitialValues({});
      final config = await configRepo();
