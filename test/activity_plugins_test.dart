...
void main() {
  ...
  group('FfmLiveActivitySensePlugin', () {
    ...
    test(
      'returns structured root and child activity breakdown when active',
      () async {
        ...
        expect(
          result.metadata['activity_payload_type'],
          equals('live_activity'),
        );
      },
    );
  });
  ...
  group('FfmActivityGuardPlugin', () {
    ...
    test(
      'warns when user attempts to finish parent while child is still running',
      () async {
  ...
  group('FfmQuickNoteActuatorPlugin', () {
    ...
    test(
      'calculates total area from notes: 1200 m2 + 500 m2 = 1700 m2',
      () async {
        final now = DateTime.now();
  ...
  group('FfmWeeklyActivityReportPlugin', () {
    test(
      'separates root from nested sessions to prevent double-counting',
      () async {
