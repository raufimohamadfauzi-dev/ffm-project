...
void main() {
  ...
  test('Soft delete transaksi dan pemulihan (undo) mengembalikan status aktif serta mencatat audit log', () async {
    ...
    var txRow = await (database.select(
    ...
    await (database.update(
      database.transactions,
    )..where((t) => t.id.equals('tx-1'))).write(
    ...
    expect(
      logs.any((log) => log.action == 'hapus' && log.entity == 'transaksi'),
      isTrue,
    );
    expect(
      logs.any((log) => log.action == 'pulihkan' && log.entity == 'transaksi'),
      isTrue,
    );
  });

  test(
    'Penghapusan transfer beserta biaya admin dan pemulihan (undo) keduanya',
    () async {
      final now = DateTime(2026, 9, 3);
      const householdId = 'household-test';
      ...
      await (database.update(database.transfers)
            ..where((t) => t.id.equals('trf-1')))
          .write(const TransfersCompanion(isDeleted: Value(true)));
      await (database.update(database.transactions)
            ..where((t) => t.id.equals('fee-1')))
          .write(const TransactionsCompanion(isDeleted: Value(true)));

      var trfRow = await (database.select(
        database.transfers,
      )..where((t) => t.id.equals('trf-1'))).getSingle();
      var feeRow = await (database.select(
        database.transactions,
      )..where((t) => t.id.equals('fee-1'))).getSingle();
      ...
      trfRow = await (database.select(
        database.transfers,
      )..where((t) => t.id.equals('trf-1'))).getSingle();
      feeRow = await (database.select(
        database.transactions,
      )..where((t) => t.id.equals('fee-1'))).getSingle();
      ...
      final logs = await auditRepo.getLogs(householdId: AppContext.householdId);
      expect(
        logs.any((log) => log.action == 'pulihkan' && log.entity == 'transfer'),
        isTrue,
      );
    },
  );
