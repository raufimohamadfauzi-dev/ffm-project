...
void main() {
  group('FfmFollowUpSuggestionEngine', () {
    ...
    test(
      'generates 3 vehicle/fuel questions when topic mentions bbm or bensin',
      () {
        final suggestions = engine.generateSuggestions(
          userText: 'Saya baru isi bensin motor Rp 50.000',
          assistantResponse:
              'Catatan pengeluaran bensin Pertalite berhasil disimpan.',
        );
    ...
    test('generates 3 electricity/token questions when topic mentions token listrik', () {
      final suggestions = engine.generateSuggestions(
        ...
        assistantResponse:
            'Token PLN sebesar Rp 100.000 untuk meteran Ruko telah dicatat.',
      );
    ...
    test(
      'generates 3 debt payoff questions when topic mentions hutang or cicilan',
      () {
        final suggestions = engine.generateSuggestions(
          userText: 'Berapa total hutang dan cicilan saya?',
          assistantResponse:
              'Total kewajiban aktif Anda saat ini adalah Rp 12.500.000.',
        );
    ...
    test('generates 3 default cash overview questions when topic is generic', () {
      final suggestions = engine.generateSuggestions(
        ...
        assistantResponse:
            'Halo! Ada yang bisa saya bantu untuk keuangan keluarga hari ini?',
      );
