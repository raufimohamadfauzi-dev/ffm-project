...
import 'package:ffm_manager/shared/widgets/hijri_date_components.dart';
...
void main() {
  ...
  Widget buildTestApp({Size? physicalSize}) {
    return MaterialApp(home: const TransactionListPage());
  }

  testWidgets(
    'tanggal berada di bawah toko dengan lebar penuh di layar kecil',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(360, 800));
      addTearDown(() => tester.binding.setSurfaceSize(null));
      final today = DateTime.now();
      final now = DateTime(today.year, today.month, today.day, 17, 50);
      await db
          .into(db.merchants)
          .insert(
      ...
      await db
          ...
          .insert(
            TransactionsCompanion.insert(
              ...
              recordedAt: now,
              createdAt: now,
              merchantId: const drift.Value('shope'),
            ),
          );
      await tester.pumpWidget(buildTestApp());
      await tester.pumpAndSettle();
      final merchant = find.text('Shope');
      await tester.ensureVisible(merchant);
      final date = find.byWidgetPredicate(
        (widget) => widget is HijriDateText && widget.date == now,
      );
      expect(date, findsOneWidget);
      expect(
        tester.getTopLeft(date).dy,
        greaterThan(tester.getBottomLeft(merchant).dy),
      );
      expect(tester.getSize(date).width, greaterThan(250));
      expect(tester.takeException(), isNull);
    },
  );

  testWidgets(
    'AppBar TransactionListPage tidak menampilkan icon Asisten dan memiliki judul Transaksi',
    (tester) async {
      await tester.pumpWidget(buildTestApp());
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'Timeline merender pemasukan, pengeluaran, dan transfer tanpa transfer mengubah total arus kas',
    (tester) async {
      ...
      expect(
        find.text(
          '1 transfer tidak mengubah total pemasukan atau pengeluaran.',
        ),
        findsOneWidget,
      );
    },
  ...
  testWidgets(
    'Tampilan tetap rapi dan tidak overflow pada lebar layar kecil 320dp',
    (tester) async {
