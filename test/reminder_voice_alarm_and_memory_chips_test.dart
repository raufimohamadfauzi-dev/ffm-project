...
void main() {
  ...
  group('Personal Memory Chips in Assistant UI', () {
    testWidgets(
      'Renders memory chip when usedMemories is not empty and triggers onOpenMemoryViewer',
      (tester) async {
        ...
        expect(
          find.text('🧠 Memori: Komoditas tani/usaha: pepaya'),
          findsOneWidget,
        );
  ...
  group('Talking Voice Alarm Ringing Dialog', () {
    testWidgets('Displays alarm details, destination button, and completes', (
      tester,
    ) async {
      ...
      expect(
        find.text('Cek kondisi buah dan timbang hasil panen'),
        findsOneWidget,
      );
    ...
    testWidgets(
      'Defaults destination button to Halaman Pengingat when destinationRoute is null',
      (tester) async {
        String? routedTo;
    ...
    testWidgets(
      'Auto-Silence automatically triggers onSnooze when snoozeCount < 3',
      (tester) async {
    ...
    testWidgets(
      'Auto-Silence terminates with onComplete and hides snooze button when snoozeCount >= 3',
      (tester) async {
  ...
  group('Assistant Reminder Intent & Title Extraction', () {
    ...
    test('ingatkan saya besok jam 6 pagi mau ke kebun -> mode notifikasi, judul bersih Ke Kebun, route activity', () async {
      final intent = await interpreter.interpret(
        'ingatkan saya besok jam 6 pagi mau ke kebun',
      );
    ...
    test('bangunkan saya besok jam 6 pagi mau ke kebun -> mode alarm otomatis, judul Ke Kebun, route activity', () async {
      final intent = await interpreter.interpret(
        'bangunkan saya besok jam 6 pagi mau ke kebun',
      );
      expect(intent.type, FfmAssistantIntentType.createReminder);
      final draft = intent.draft;
      expect(draft, isNotNull);
    ...
    test(
      'ingatkan bayar tagihan listrik besok jam 7 malam -> route liabilities',
      () async {
        final intent = await interpreter.interpret(
          'ingatkan bayar tagihan listrik besok jam 7 malam',
        );
        expect(intent.type, FfmAssistantIntentType.createReminder);
        final draft = intent.draft;
        expect(draft, isNotNull);
    ...
    test(
      'ingatkan saya target beli emas besok jam 9 pagi -> route goals',
      () async {
        final intent = await interpreter.interpret(
          'ingatkan saya target beli emas besok jam 9 pagi',
        );
        expect(intent.type, FfmAssistantIntentType.createReminder);
        final draft = intent.draft;
        expect(draft, isNotNull);
