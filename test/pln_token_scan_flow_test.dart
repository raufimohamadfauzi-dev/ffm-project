...
void main() {
  ...
  group('PLN Electricity Token OCR Flow Tests', () {
    ...
    test(
      'Deteksi token 20-digit dan memperbarui meteran yang cocok di repository',
      () async {
        ...
        final meterRegex = RegExp(
          r'(?:idpel|meter|no\.?\s*meter|nomor\s*meteran?)[:\s]*(\d{11,12})',
          caseSensitive: false,
        );
        ...
        final target = updatedMeters.firstWhere(
          (m) => m.id == existingMeter.id,
        );
    ...
    test(
      'Auto-register meteran baru jika nomor meteran belum ada di Buku Saku',
      () async {
        final repo = UtilityMeterRepository();
        const newMeterNumber = '14987654321';
        const cleanToken = '98765432109876543210';

        final matched = await repo.findMeterByNumber(
          householdId,
          newMeterNumber,
        );
