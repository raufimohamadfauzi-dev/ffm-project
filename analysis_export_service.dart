...
class AnalysisExportService {
  ...
  AnalysisExportBundle build({
    ...
    final filtered = records.where((entry) {
      ...
      if (to != null && date.isAfter(to.add(const Duration(days: 1)))) return false;
      if (typeFilter != 'all' && transaction.type != typeFilter) return false;
      if (categoryFilter != 'all' && transaction.categoryId != categoryFilter) return false;
      return true;
    }).toList();
    final rows = filtered.map((entry) => _row(
          entry,
    ...
    final prompt = _prompt(periodLabel, rows.length, includeNotes, includeItems);
    return AnalysisExportBundle(periodLabel: periodLabel, json: json, csv: csv, html: html, prompt: prompt);
  }
  ...
  Map<String, Object?> _row(
    ...
    return {
      ...
      'kategori': categoryLabels[transaction.categoryId] ?? transaction.categoryId ?? 'Belum diatur',
      'rekening': transaction.accountId,
      'toko': merchantLabels[merchantId] ?? merchantId,
      if (includeMerchantDetails && merchantId != null) 'detailToko': merchantDetails[merchantId],
      'dipakaiOlehAtauSumber': transaction.partyName,
      'lokasi': transaction.location,
      if (includeNotes) 'catatan': transaction.note,
      if (linkedTags.isNotEmpty) 'tag': linkedTags.map((id) => tagLabels[id] ?? id).toList(),
      if (includeItems)
        'rincian': entry.items.map((item) => {
              'nama': item.itemName,
              'jumlah': item.qty,
              'satuan': item.unit,
              'harga': item.price,
  ...
  String _toCsv(List<Map<String, Object?>> rows) {
    const headers = ['tanggal', 'jenis', 'nominal', 'kategori', 'rekening', 'toko', 'dipakaiOlehAtauSumber', 'lokasi', 'catatan'];
  ...
  String _csvLine(Iterable<Object?> values) => values.map((value) {
        final text = value?.toString() ?? '';
        return '"${text.replaceAll('"', '""')}"';
      }).join(',');

  String _toHtml(List<Map<String, Object?>> rows, String periodLabel) {
    final body = rows.map((row) => '<tr><td>${_escape(row['tanggal'])}</td><td>${_escape(row['jenis'])}</td><td>${_escape(row['nominal'])}</td><td>${_escape(row['kategori'])}</td><td>${_escape(row['toko'])}</td></tr>').join();
  ...
  String _escape(Object? value) => const HtmlEscape().convert(value?.toString() ?? '');

  String _periodLabel(DateTime? from, DateTime? to) {
    if (from == null && to == null) return 'Semua periode';
    final start = from == null ? 'awal' : '${from.day}/${from.month}/${from.year}';
    final end = to == null ? 'sekarang' : '${to.day}/${to.month}/${to.year}';
    return '$start sampai $end';
  }

  String _prompt(String periodLabel, int count, bool includeNotes, bool includeItems) => '''Kamu adalah pendamping analisa keuangan keluarga. Analisa data FFM berikut secara hati-hati dan jangan mengarang angka.
